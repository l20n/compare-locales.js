...
function createWarning(elem, name, suggestions) {
  ...
  if (suggestions[1]) {
    var msg = suggestions[0] + ' is misspelled. Suggested spellings: ' +
      suggestions[1].join(', ');
    return ['spellcheck', levels.WARNING, elem, name, msg];
  }

  return [
    'spellcheck', levels.WARNING, elem, name,
    suggestions[0] + ' is misspelled'];
}
...
module.exports = function(evt) {
  // don't do anything if the user doesn't want to see warnings
  if (evt.severity > levels.WARNING) {
    return;
  }
