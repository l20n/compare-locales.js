...
GaiaApp.enableChecks = function(emitter) {
  if (!checksEnabled) {
    emitter.on('compare-entries', require('./placeables'));
    emitter.on('compare-entries', require('./spellcheck'));
    checksEnabled = true;
  }
};

GaiaApp.disableChecks = function() {
};
