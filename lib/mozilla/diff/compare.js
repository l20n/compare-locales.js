...
function Comparer(config, App) {
  ...
  var hooks = config.runTests.split(',').map(function(test) {
    return test.trim();
  });
...
function compareValues(field1, field2) {
  ...
  if (field1.value !== null && field2.value !== null) {
    if (deepEqual(field1.value, field2.value) &&
        deepEqual(field1.index, field2.index)) {
      return ['builtin', 'unchanged', 'value'];
    } else {
      if (isResolvable(field1) !== isResolvable(field2)) {
