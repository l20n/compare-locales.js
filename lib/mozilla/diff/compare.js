...
function parseEntryListDiffElem(entrylist1, entrylist2, elem) {
  ...
  switch(elem[1]) {
    case 'list1':
      return ['missing', entryId];
    case 'list2':
      return ['obsolete', entryId];
    case 'both':
      return this.compareEntries(
        entrylist1[entryId], entrylist2[entryId], entryId).then(
          function(entryDiff) {
            return ['present', entryId, entryDiff];
          });
  }
}
...
Comparer.prototype.compareEntries = function(entry1, entry2, entryId) {
  ...
  this.emitter.emit('compare-entries', {
    entry1: entry1,
    entry2: entry2,
    callback: function(hooksWarnings) {
      hooksWarnings.forEach(function(warning) {
  ...
  return Promise.all(warnings).then(function(resolvedWarnings) {
    entryDiff.warnings = resolvedWarnings.filter(isNotNull);
    return entryDiff;
  });
};
